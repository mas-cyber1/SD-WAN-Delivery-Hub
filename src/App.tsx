...
function InventoryWorkspace({ token, clients, projects, sites, devices, circuits, networks, vlans, interfaces, onInventoryChanged }: { token: string; clients: ClientRecord[]; projects: ProjectRecord[]; sites: SiteRecord[]; devices: DeviceRecord[]; circuits: CircuitRecord[]; networks: NetworkRecord[]; vlans: VlanRecord[]; interfaces: InterfaceRecord[]; onInventoryChanged: () => Promise<void> }) {
  ...
  async function createDevice(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setError(''); setSubmitting(true)
    try {
      const response = await fetch('/api/inventory/devices', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ ...deviceForm, site_id: Number(deviceForm.site_id), vendor: deviceForm.vendor || null, model: deviceForm.model || null, management_ip: deviceForm.management_ip || null, description: deviceForm.description || null }) })
      if (!response.ok) throw new Error('Unable to create device')
      setDeviceForm({ site_id: '', hostname: '', role: 'sdwan_edge', vendor: '', model: '', management_ip: '', status: 'planned', description: '' }); await onInventoryChanged()
    } catch (submitError) { setError(submitError instanceof Error ? submitError.message : 'Unable to create device') } finally { setSubmitting(false) }
  }

  async function createCircuit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setError(''); setSubmitting(true)
    try {
      const response = await fetch('/api/inventory/circuits', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ ...circuitForm, site_id: Number(circuitForm.site_id), bandwidth_mbps: circuitForm.bandwidth_mbps ? Number(circuitForm.bandwidth_mbps) : null, public_ip: circuitForm.public_ip || null, description: circuitForm.description || null }) })
      if (!response.ok) throw new Error('Unable to create WAN circuit')
      setCircuitForm({ site_id: '', name: '', provider: '', circuit_type: 'internet', role: 'primary', bandwidth_mbps: '', public_ip: '', status: 'planned', description: '' }); await onInventoryChanged()
    } catch (submitError) { setError(submitError instanceof Error ? submitError.message : 'Unable to create WAN circuit') } finally { setSubmitting(false) }
  }

  async function createNetwork(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setError(''); setSubmitting(true)
    try {
      const response = await fetch('/api/inventory/networks', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ ...networkForm, site_id: Number(networkForm.site_id), gateway: networkForm.gateway || null }) })
      if (!response.ok) throw new Error('Unable to create IP network')
      setNetworkForm({ site_id: '', name: '', cidr: '', gateway: '', network_type: 'lan', status: 'planned' }); await onInventoryChanged()
    } catch (submitError) { setError(submitError instanceof Error ? submitError.message : 'Unable to create IP network') } finally { setSubmitting(false) }
  }

  async function createVlan(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setError(''); setSubmitting(true)
    try {
      const response = await fetch('/api/inventory/vlans', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ ...vlanForm, site_id: Number(vlanForm.site_id), vlan_id: Number(vlanForm.vlan_id), subnet: vlanForm.subnet || null, gateway: vlanForm.gateway || null }) })
      if (!response.ok) throw new Error('Unable to create VLAN')
      setVlanForm({ site_id: '', vlan_id: '', name: '', subnet: '', gateway: '', status: 'planned' }); await onInventoryChanged()
    } catch (submitError) { setError(submitError instanceof Error ? submitError.message : 'Unable to create VLAN') } finally { setSubmitting(false) }
  }

  async function createInterface(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setError(''); setSubmitting(true)
    try {
      const response = await fetch('/api/inventory/interfaces', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ ...interfaceForm, site_id: Number(interfaceForm.site_id), device_id: Number(interfaceForm.device_id), ip_address: interfaceForm.ip_address || null, connected_to: interfaceForm.connected_to || null }) })
      if (!response.ok) throw new Error('Unable to create interface')
      setInterfaceForm({ site_id: '', device_id: '', name: '', interface_role: 'lan', ip_address: '', connected_to: '', status: 'planned' }); await onInventoryChanged()
    } catch (submitError) { setError(submitError instanceof Error ? submitError.message : 'Unable to create interface') } finally { setSubmitting(false) }
  }

  const groups = clients.map((client) => ({ client, projects: projects.filter((project) => project.client_id === client.id).map((project) => ({ project, sites: sites.filter((site) => site.project_id === project.id).map((site) => ({ site, devices: devices.filter((device) => device.site_id === site.id), circuits: circuits.filter((circuit) => circuit.site_id === site.id), siteNetworks: networks.filter((network) => network.site_id === site.id), siteVlans: vlans.filter((vlan) => vlan.site_id === site.id), siteInterfaces: interfaces.filter((networkInterface) => networkInterface.site_id === site.id) })).filter(({ devices: siteDevices, circuits: siteCircuits, siteNetworks, siteVlans, siteInterfaces }) => siteDevices.length > 0 || siteCircuits.length > 0 || siteNetworks.length > 0 || siteVlans.length > 0 || siteInterfaces.length > 0) })).filter(({ sites: projectSites }) => projectSites.length > 0) })).filter(({ projects: clientProjects }) => clientProjects.length > 0)

  return <div className="workflow-layout"><section className="panel workflow-register"><div className="panel-heading"><div><h3>Inventory by site</h3><p className="muted">Devices, circuits, networks, VLANs, and interfaces grouped under Client → Project → Site.</p></div><span className="panel-label">{devices.length + circuits.length + networks.length + vlans.length + interfaces.length} total</span></div>{groups.length === 0 ? <div className="empty-state compact"><Network size={29} /><span>No network inventory recorded</span></div> : <div className="hierarchy-stack">{groups.map(({ client, projects: clientProjects }) => <div key={client.id} className="hierarchy-card"><div className="hierarchy-header"><div><strong>{client.name}</strong><span>{client.client_code}</span></div><span className="muted-tag">Client inventory</span></div>{clientProjects.map(({ project, sites: projectSites }) => <div key={project.id} className="nested-project"><div className="hierarchy-header"><div><strong>{project.name}</strong><span>{project.project_code}</span></div></div>{projectSites.map(({ site, devices: siteDevices, circuits: siteCircuits, siteNetworks, siteVlans, siteInterfaces }) => <div key={site.id} className="inventory-site"><div className="hierarchy-header"><div><strong>{site.name}</strong><span>{site.site_code}</span></div><span className="muted-tag">{siteDevices.length} devices · {siteCircuits.length} circuits</span></div>{siteDevices.map((device) => <div key={device.id} className="nested-item"><div><strong>{device.hostname}</strong><span>{device.role}{device.vendor ? ` · ${device.vendor}` : ''}{device.model ? ` ${device.model}` : ''}{device.management_ip ? ` · ${device.management_ip}` : ''}</span></div><span className={`status-badge ${device.status}`}>{device.status}</span></div>)}{siteCircuits.map((circuit) => <div key={circuit.id} className="nested-item"><div><strong>{circuit.name}</strong><span>{circuit.provider} · {circuit.circuit_type}{circuit.bandwidth_mbps ? ` · ${circuit.bandwidth_mbps} Mbps` : ''}</span></div><span className={`status-badge ${circuit.status}`}>{circuit.role}</span></div>)}{siteNetworks.map((network) => <div key={network.id} className="nested-item"><div><strong>{network.name}</strong><span>{network.network_type} · {network.cidr}{network.gateway ? ` · gateway ${network.gateway}` : ''}</span></div><span className={`status-badge ${network.status}`}>{network.status}</span></div>)}{siteVlans.map((vlan) => <div key={vlan.id} className="nested-item"><div><strong>VLAN {vlan.vlan_id} · {vlan.name}</strong><span>{vlan.subnet ?? 'No subnet'}{vlan.gateway ? ` · gateway ${vlan.gateway}` : ''}</span></div><span className={`status-badge ${vlan.status}`}>{vlan.status}</span></div>)}{siteInterfaces.map((networkInterface) => <div key={networkInterface.id} className="nested-item"><div><strong>{networkInterface.name}</strong><span>{networkInterface.interface_role}{networkInterface.ip_address ? ` · ${networkInterface.ip_address}` : ''}{networkInterface.connected_to ? ` · to ${networkInterface.connected_to}` : ''}</span></div><span className={`status-badge ${networkInterface.status}`}>{networkInterface.status}</span></div>)}</div>)}</div>)}</div>)}</div>}
  </section><section className="panel"><div className="panel-heading"><div><h3>Add inventory</h3><p className="muted">Capture the facts used later for diagrams and LLDs.</p></div></div>{error && <p className="form-error">{error}</p>}<form className="form-grid" onSubmit={createDevice}><h4>New device</h4><label>Site<select value={deviceForm.site_id} onChange={(event) => setDeviceForm({ ...deviceForm, site_id: event.target.value })} required><option value="">Select site</option>{sites.map((site) => <option key={site.id} value={String(site.id)}>{site.name}</option>)}</select></label><label>Hostname<input value={deviceForm.hostname} onChange={(event) => setDeviceForm({ ...deviceForm, hostname: event.target.value })} required /></label><label>Role<select value={deviceForm.role} onChange={(event) => setDeviceForm({ ...deviceForm, role: event.target.value })}><option value="sdwan_edge">SD-WAN edge</option><option value="firewall">Firewall</option><option value="core_switch">Core switch</option><option value="access_switch">Access switch</option><option value="controller">Controller</option></select></label><label>Vendor<input value={deviceForm.vendor} onChange={(event) => setDeviceForm({ ...deviceForm, vendor: event.target.value })} /></label><label>Model<input value={deviceForm.model} onChange={(event) => setDeviceForm({ ...deviceForm, model: event.target.value })} /></label><label>Management IP<input value={deviceForm.management_ip} onChange={(event) => setDeviceForm({ ...deviceForm, management_ip: event.target.value })} /></label><label>Status<select value={deviceForm.status} onChange={(event) => setDeviceForm({ ...deviceForm, status: event.target.value })}><option value="planned">Planned</option><option value="ordered">Ordered</option><option value="staged">Staged</option><option value="deployed">Deployed</option></select></label><label>Description<textarea value={deviceForm.description} onChange={(event) => setDeviceForm({ ...deviceForm, description: event.target.value })} rows={2} /></label><button className="primary-button" disabled={submitting}>{submitting ? 'Saving...' : 'Save device'}</button></form><form className="form-grid workflow-form-divider" onSubmit={createCircuit}><h4>New WAN circuit</h4><label>Site<select value={circuitForm.site_id} onChange={(event) => setCircuitForm({ ...circuitForm, site_id: event.target.value })} required><option value="">Select site</option>{sites.map((site) => <option key={site.id} value={String(site.id)}>{site.name}</option>)}</select></label><label>Circuit name<input value={circuitForm.name} onChange={(event) => setCircuitForm({ ...circuitForm, name: event.target.value })} required /></label><label>Provider<input value={circuitForm.provider} onChange={(event) => setCircuitForm({ ...circuitForm, provider: event.target.value })} required /></label><label>Type<select value={circuitForm.circuit_type} onChange={(event) => setCircuitForm({ ...circuitForm, circuit_type: event.target.value })}><option value="internet">Internet</option><option value="mpls">MPLS</option><option value="private_ethernet">Private Ethernet</option><option value="lte_5g">LTE / 5G</option></select></label><label>Role<select value={circuitForm.role} onChange={(event) => setCircuitForm({ ...circuitForm, role: event.target.value })}><option value="primary">Primary</option><option value="backup">Backup</option><option value="tertiary">Tertiary</option></select></label><label>Bandwidth (Mbps)<input type="number" min={0} value={circuitForm.bandwidth_mbps} onChange={(event) => setCircuitForm({ ...circuitForm, bandwidth_mbps: event.target.value })} /></label><label>Public IP<input value={circuitForm.public_ip} onChange={(event) => setCircuitForm({ ...circuitForm, public_ip: event.target.value })} /></label><label>Status<select value={circuitForm.status} onChange={(event) => setCircuitForm({ ...circuitForm, status: event.target.value })}><option value="planned">Planned</option><option value="ordered">Ordered</option><option value="provisioning">Provisioning</option><option value="live">Live</option></select></label><button className="primary-button" disabled={submitting}>{submitting ? 'Saving...' : 'Save circuit'}</button></form><form className="form-grid workflow-form-divider" onSubmit={createNetwork}><h4>New IP network</h4><label>Site<select value={networkForm.site_id} onChange={(event) => setNetworkForm({ ...networkForm, site_id: event.target.value })} required><option value="">Select site</option>{sites.map((site) => <option key={site.id} value={String(site.id)}>{site.name}</option>)}</select></label><label>Name<input value={networkForm.name} onChange={(event) => setNetworkForm({ ...networkForm, name: event.target.value })} required /></label><label>CIDR<input placeholder="10.10.0.0/24" value={networkForm.cidr} onChange={(event) => setNetworkForm({ ...networkForm, cidr: event.target.value })} required /></label><label>Gateway<input value={networkForm.gateway} onChange={(event) => setNetworkForm({ ...networkForm, gateway: event.target.value })} /></label><label>Type<select value={networkForm.network_type} onChange={(event) => setNetworkForm({ ...networkForm, network_type: event.target.value })}><option value="lan">LAN</option><option value="wan">WAN</option><option value="management">Management</option><option value="loopback">Loopback</option></select></label><button className="primary-button" disabled={submitting}>Save network</button></form><form className="form-grid workflow-form-divider" onSubmit={createVlan}><h4>New VLAN</h4><label>Site<select value={vlanForm.site_id} onChange={(event) => setVlanForm({ ...vlanForm, site_id: event.target.value })} required><option value="">Select site</option>{sites.map((site) => <option key={site.id} value={String(site.id)}>{site.name}</option>)}</select></label><label>VLAN ID<input type="number" min={1} max={4094} value={vlanForm.vlan_id} onChange={(event) => setVlanForm({ ...vlanForm, vlan_id: event.target.value })} required /></label><label>Name<input value={vlanForm.name} onChange={(event) => setVlanForm({ ...vlanForm, name: event.target.value })} required /></label><label>Subnet<input value={vlanForm.subnet} onChange={(event) => setVlanForm({ ...vlanForm, subnet: event.target.value })} /></label><label>Gateway<input value={vlanForm.gateway} onChange={(event) => setVlanForm({ ...vlanForm, gateway: event.target.value })} /></label><button className="primary-button" disabled={submitting}>Save VLAN</button></form><form className="form-grid workflow-form-divider" onSubmit={createInterface}><h4>New interface</h4><label>Site<select value={interfaceForm.site_id} onChange={(event) => setInterfaceForm({ ...interfaceForm, site_id: event.target.value, device_id: '' })} required><option value="">Select site</option>{sites.map((site) => <option key={site.id} value={String(site.id)}>{site.name}</option>)}</select></label><label>Device<select value={interfaceForm.device_id} onChange={(event) => setInterfaceForm({ ...interfaceForm, device_id: event.target.value })} required><option value="">Select device</option>{devices.filter((device) => device.site_id === Number(interfaceForm.site_id)).map((device) => <option key={device.id} value={String(device.id)}>{device.hostname}</option>)}</select></label><label>Interface<input placeholder="ge-0/0/0" value={interfaceForm.name} onChange={(event) => setInterfaceForm({ ...interfaceForm, name: event.target.value })} required /></label><label>Role<select value={interfaceForm.interface_role} onChange={(event) => setInterfaceForm({ ...interfaceForm, interface_role: event.target.value })}><option value="lan">LAN</option><option value="wan">WAN</option><option value="management">Management</option><option value="loopback">Loopback</option></select></label><label>IP address<input value={interfaceForm.ip_address} onChange={(event) => setInterfaceForm({ ...interfaceForm, ip_address: event.target.value })} /></label><label>Connected to<input value={interfaceForm.connected_to} onChange={(event) => setInterfaceForm({ ...interfaceForm, connected_to: event.target.value })} /></label><button className="primary-button" disabled={submitting}>Save interface</button></form></section></div>
}
