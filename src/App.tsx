...
type HubRecord = { id: number; project_id: number; site_id: number; device_id: number | null; name: string; hub_type: string; controller_hostname: string | null; region: string | null; status: string }
...
function AuthenticatedApp({ token, userName, onLogout }: { token: string; userName: string; onLogout: () => void }) {
  ...
  const loadOverlay = async () => {
    const headers = { Authorization: `Bearer ${token}` }
    const [hubsResponse, tunnelsResponse] = await Promise.all([fetch('/api/overlay/hubs', { headers }), fetch('/api/overlay/tunnels', { headers })])
    if (hubsResponse.ok) setHubs((await hubsResponse.json()) as HubRecord[])
    if (tunnelsResponse.ok) setTunnels((await tunnelsResponse.json()) as TunnelRecord[])
  }
...
function OverlayWorkspace({ token, projects, sites, devices, hubs, tunnels, onOverlayChanged }: { token: string; projects: ProjectRecord[]; sites: SiteRecord[]; devices: DeviceRecord[]; hubs: HubRecord[]; tunnels: TunnelRecord[]; onOverlayChanged: () => Promise<void> }) {
  const [hubForm, setHubForm] = useState({ project_id: '', site_id: '', device_id: '', name: '', hub_type: 'hub', controller_hostname: '', region: '', status: 'planned' })
  const [tunnelForm, setTunnelForm] = useState({ project_id: '', source_site_id: '', destination_site_id: '', source_device_id: '', destination_device_id: '', name: '', path_role: 'primary', transport: 'internet', local_tunnel_ip: '', remote_tunnel_ip: '', status: 'planned' })
  const [error, setError] = useState('')
  const [submitting, setSubmitting] = useState(false)

  async function submitHub(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setError(''); setSubmitting(true)
    const response = await fetch('/api/overlay/hubs', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ ...hubForm, project_id: Number(hubForm.project_id), site_id: Number(hubForm.site_id), device_id: hubForm.device_id ? Number(hubForm.device_id) : null, controller_hostname: hubForm.controller_hostname || null, region: hubForm.region || null }) })
    if (!response.ok) { const payload = (await response.json().catch(() => ({ detail: 'Unable to create hub' }))) as { detail?: string }; setError(payload.detail ?? 'Unable to create hub'); setSubmitting(false); return }
    setHubForm({ project_id: '', site_id: '', device_id: '', name: '', hub_type: 'hub', controller_hostname: '', region: '', status: 'planned' }); setSubmitting(false); await onOverlayChanged()
  }

  async function submitTunnel(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setError(''); setSubmitting(true)
    const response = await fetch('/api/overlay/tunnels', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ ...tunnelForm, project_id: Number(tunnelForm.project_id), source_site_id: Number(tunnelForm.source_site_id), destination_site_id: Number(tunnelForm.destination_site_id), source_device_id: tunnelForm.source_device_id ? Number(tunnelForm.source_device_id) : null, destination_device_id: tunnelForm.destination_device_id ? Number(tunnelForm.destination_device_id) : null, local_tunnel_ip: tunnelForm.local_tunnel_ip || null, remote_tunnel_ip: tunnelForm.remote_tunnel_ip || null }) })
    if (!response.ok) { const payload = (await response.json().catch(() => ({ detail: 'Unable to create tunnel' }))) as { detail?: string }; setError(payload.detail ?? 'Unable to create tunnel'); setSubmitting(false); return }
    setTunnelForm({ project_id: '', source_site_id: '', destination_site_id: '', source_device_id: '', destination_device_id: '', name: '', path_role: 'primary', transport: 'internet', local_tunnel_ip: '', remote_tunnel_ip: '', status: 'planned' }); setSubmitting(false); await onOverlayChanged()
  }

  const siteName = (siteId: number) => sites.find((site) => site.id === siteId)?.name ?? 'Unknown site'
  const projectTunnels = projects.map((project) => ({ project, tunnels: tunnels.filter((tunnel) => tunnel.project_id === project.id), projectHubs: hubs.filter((hub) => hub.project_id === project.id) })).filter(({ tunnels: projectTunnelsForProject, projectHubs }) => projectTunnelsForProject.length > 0 || projectHubs.length > 0)
  const hubSites = sites.filter((site) => site.project_id === Number(hubForm.project_id))
  const tunnelSites = sites.filter((site) => site.project_id === Number(tunnelForm.project_id))
  const hubDevices = devices.filter((device) => device.site_id === Number(hubForm.site_id))
  const sourceDevices = devices.filter((device) => device.site_id === Number(tunnelForm.source_site_id))
  const destinationDevices = devices.filter((device) => device.site_id === Number(tunnelForm.destination_site_id))

  return <div className="workspace-grid"><section className="panel workflow-register"><div className="panel-heading"><div><h3>Routing and overlay by project</h3><p className="muted">Project hubs and site-to-site SD-WAN paths for future diagrams.</p></div><span className="panel-label">{hubs.length} hubs · {tunnels.length} tunnels</span></div>{projectTunnels.length === 0 ? <div className="empty-state compact"><Network size={29} /><span>No routing or overlay records yet</span></div> : <div className="hierarchy-stack">{projectTunnels.map(({ project, projectHubs, tunnels: projectTunnelsForProject }) => <div key={project.id} className="hierarchy-card"><div className="hierarchy-header"><div><strong>{project.name}</strong><span>{project.project_code}</span></div><span className="muted-tag">Project overlay</span></div>{projectHubs.map((hub) => <div key={hub.id} className="nested-item"><div><strong>{hub.name}</strong><span>{siteName(hub.site_id)} · {hub.hub_type}{hub.controller_hostname ? ` · ${hub.controller_hostname}` : ''}</span></div><span className={`status-badge ${hub.status}`}>{hub.status}</span></div>)}{projectTunnelsForProject.map((tunnel) => <div key={tunnel.id} className="nested-item"><div><strong>{tunnel.name}</strong><span>{siteName(tunnel.source_site_id)} to {siteName(tunnel.destination_site_id)} · {tunnel.transport} · {tunnel.path_role}</span></div><span className={`status-badge ${tunnel.status}`}>{tunnel.status}</span></div>)}</div>)}</div>}
  </section><section className="panel"><div className="panel-heading"><div><h3>Add overlay record</h3><p className="muted">Define a project hub or a primary/backup tunnel path.</p></div></div>{error && <p className="form-error">{error}</p>}<form className="form-grid" onSubmit={submitHub}><h4>New hub or controller</h4><label>Project<select value={hubForm.project_id} onChange={(event) => setHubForm({ ...hubForm, project_id: event.target.value, site_id: '', device_id: '' })} required><option value="">Select project</option>{projects.map((project) => <option key={project.id} value={String(project.id)}>{project.name}</option>)}</select></label><label>Site<select value={hubForm.site_id} onChange={(event) => setHubForm({ ...hubForm, site_id: event.target.value, device_id: '' })} required><option value="">Select site</option>{hubSites.map((site) => <option key={site.id} value={String(site.id)}>{site.name}</option>)}</select></label><label>Device<select value={hubForm.device_id} onChange={(event) => setHubForm({ ...hubForm, device_id: event.target.value })}><option value="">Optional device</option>{hubDevices.map((device) => <option key={device.id} value={String(device.id)}>{device.hostname}</option>)}</select></label><label>Name<input value={hubForm.name} onChange={(event) => setHubForm({ ...hubForm, name: event.target.value })} required /></label><label>Type<select value={hubForm.hub_type} onChange={(event) => setHubForm({ ...hubForm, hub_type: event.target.value })}><option value="hub">Hub</option><option value="controller">Controller</option><option value="gateway">Gateway</option></select></label><label>Controller hostname<input value={hubForm.controller_hostname} onChange={(event) => setHubForm({ ...hubForm, controller_hostname: event.target.value })} /></label><label>Region<input value={hubForm.region} onChange={(event) => setHubForm({ ...hubForm, region: event.target.value })} /></label><label>Status<select value={hubForm.status} onChange={(event) => setHubForm({ ...hubForm, status: event.target.value })}><option value="planned">Planned</option><option value="active">Active</option><option value="retired">Retired</option></select></label><button className="primary-button" disabled={submitting}>{submitting ? 'Saving...' : 'Save hub'}</button></form><form className="form-grid workflow-form-divider" onSubmit={submitTunnel}><h4>New site-to-site tunnel</h4><label>Project<select value={tunnelForm.project_id} onChange={(event) => setTunnelForm({ ...tunnelForm, project_id: event.target.value, source_site_id: '', destination_site_id: '' })} required><option value="">Select project</option>{projects.map((project) => <option key={project.id} value={String(project.id)}>{project.name}</option>)}</select></label><label>Source site<select value={tunnelForm.source_site_id} onChange={(event) => setTunnelForm({ ...tunnelForm, source_site_id: event.target.value, source_device_id: '' })} required><option value="">Select source</option>{tunnelSites.map((site) => <option key={site.id} value={String(site.id)}>{site.name}</option>)}</select></label><label>Source device<select value={tunnelForm.source_device_id} onChange={(event) => setTunnelForm({ ...tunnelForm, source_device_id: event.target.value })}><option value="">Optional device</option>{sourceDevices.map((device) => <option key={device.id} value={String(device.id)}>{device.hostname}</option>)}</select></label><label>Destination site<select value={tunnelForm.destination_site_id} onChange={(event) => setTunnelForm({ ...tunnelForm, destination_site_id: event.target.value, destination_device_id: '' })} required><option value="">Select destination</option>{tunnelSites.filter((site) => site.id !== Number(tunnelForm.source_site_id)).map((site) => <option key={site.id} value={String(site.id)}>{site.name}</option>)}</select></label><label>Destination device<select value={tunnelForm.destination_device_id} onChange={(event) => setTunnelForm({ ...tunnelForm, destination_device_id: event.target.value })}><option value="">Optional device</option>{destinationDevices.map((device) => <option key={device.id} value={String(device.id)}>{device.hostname}</option>)}</select></label><label>Name<input value={tunnelForm.name} onChange={(event) => setTunnelForm({ ...tunnelForm, name: event.target.value })} required /></label><label>Path role<select value={tunnelForm.path_role} onChange={(event) => setTunnelForm({ ...tunnelForm, path_role: event.target.value })}><option value="primary">Primary</option><option value="backup">Backup</option><option value="tertiary">Tertiary</option></select></label><label>Transport<select value={tunnelForm.transport} onChange={(event) => setTunnelForm({ ...tunnelForm, transport: event.target.value })}><option value="internet">Internet</option><option value="mpls">MPLS</option><option value="private">Private transport</option><option value="lte_5g">LTE / 5G</option></select></label><label>Local tunnel IP<input value={tunnelForm.local_tunnel_ip} onChange={(event) => setTunnelForm({ ...tunnelForm, local_tunnel_ip: event.target.value })} /></label><label>Remote tunnel IP<input value={tunnelForm.remote_tunnel_ip} onChange={(event) => setTunnelForm({ ...tunnelForm, remote_tunnel_ip: event.target.value })} /></label><label>Status<select value={tunnelForm.status} onChange={(event) => setTunnelForm({ ...tunnelForm, status: event.target.value })}><option value="planned">Planned</option><option value="provisioning">Provisioning</option><option value="active">Active</option><option value="down">Down</option></select></label><button className="primary-button" disabled={submitting}>{submitting ? 'Saving...' : 'Save tunnel'}</button></form></section></div>
}
