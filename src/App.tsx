...
function AuthenticatedApp({ token, userName, onLogout }: { token: string; userName: string; onLogout: () => void }) {
  ...
  const loadInventory = async () => {
    const headers = { Authorization: `Bearer ${token}` }
    const [devicesResponse, circuitsResponse] = await Promise.all([fetch('/api/inventory/devices', { headers }), fetch('/api/inventory/circuits', { headers })])
...
function InventoryWorkspace({ token, clients, projects, sites, devices, circuits, onInventoryChanged }: { token: string; clients: ClientRecord[]; projects: ProjectRecord[]; sites: SiteRecord[]; devices: DeviceRecord[]; circuits: CircuitRecord[]; onInventoryChanged: () => Promise<void> }) {
  ...
  const groups = clients.map((client) => ({ client, projects: projects.filter((project) => project.client_id === client.id).map((project) => ({ project, sites: sites.filter((site) => site.project_id === project.id).map((site) => ({ site, devices: devices.filter((device) => device.site_id === site.id), circuits: circuits.filter((circuit) => circuit.site_id === site.id) })).filter(({ devices: siteDevices, circuits: siteCircuits }) => siteDevices.length > 0 || siteCircuits.length > 0) })).filter(({ sites: projectSites }) => projectSites.length > 0) })).filter(({ projects: clientProjects }) => clientProjects.length > 0)

  return <div className="workflow-layout"><section className="panel workflow-register"><div className="panel-heading"><div><h3>Inventory by site</h3><p className="muted">Devices and WAN circuits grouped under Client → Project → Site.</p></div><span className="panel-label">{devices.length + circuits.length} total</span></div>{groups.length === 0 ? <div className="empty-state compact"><Network size={29} /><span>No devices or circuits recorded</span></div> : <div className="hierarchy-stack">{groups.map(({ client, projects: clientProjects }) => <div key={client.id} className="hierarchy-card"><div className="hierarchy-header"><div><strong>{client.name}</strong><span>{client.client_code}</span></div><span className="muted-tag">Client inventory</span></div>{clientProjects.map(({ project, sites: projectSites }) => <div key={project.id} className="nested-project"><div className="hierarchy-header"><div><strong>{project.name}</strong><span>{project.project_code}</span></div></div>{projectSites.map(({ site, devices: siteDevices, circuits: siteCircuits }) => <div key={site.id} className="inventory-site"><div className="hierarchy-header"><div><strong>{site.name}</strong><span>{site.site_code}</span></div><span className="muted-tag">{siteDevices.length} devices · {siteCircuits.length} circuits</span></div>{siteDevices.map((device) => <div key={device.id} className="nested-item"><div><strong>{device.hostname}</strong><span>{device.role}{device.vendor ? ` · ${device.vendor}` : ''}{device.model ? ` ${device.model}` : ''}{device.management_ip ? ` · ${device.management_ip}` : ''}</span></div><span className={`status-badge ${device.status}`}>{device.status}</span></div>)}{siteCircuits.map((circuit) => <div key={circuit.id} className="nested-item"><div><strong>{circuit.name}</strong><span>{circuit.provider} · {circuit.circuit_type}{circuit.bandwidth_mbps ? ` · ${circuit.bandwidth_mbps} Mbps` : ''}</span></div><span className={`status-badge ${circuit.status}`}>{circuit.role}</span></div>)}</div>)}</div>)}</div>)}</div>}
